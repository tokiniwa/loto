...
RSpec.describe LotoSixEvent, type: :model do
  describe 'リレーション' do
  ...
  describe '.lottery_date_order' do
    ...
    let!(:event1) { create :loto_six_event, lottery_date: 2.days.ago }
    let!(:event2) { create :loto_six_event, lottery_date: 2.days.since }
    let!(:event3) { create :loto_six_event, lottery_date: Date.today }
