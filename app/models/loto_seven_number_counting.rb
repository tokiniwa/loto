class LotoSevenNumberCounting < ApplicationRecord
  ...
  validates :lottery_number_rate7, presence: true

  scope :number_order, -> { order(number: :asc) }

  def self.set_lottery_numbers lottery_count: 52
    event_ids = LotoEvent.loto_seven.lottery_date_order.limit(lottery_count).ids
    LotoSevenNumber.where(loto_event_id: event_ids)
                   .group([:priority, :lottery_number])
                   .count
                   .each do |k, v|
      counting = LotoSevenNumberCounting.find_by(number: k.second)
